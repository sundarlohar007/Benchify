...
### B-132 — pcprobe Cargo.toml referenced SDK crate as `sdk` but package is `performancebench-sdk`

- **Severity:** BLOCKER
- **Where:** `performancebench-injector/pcprobe/Cargo.toml:13` (pre-fix)
...
### B-133 — pcprobe references non-existent `pc_metrics` feature in SDK
...
### B-134 — `collector.rs` uses removed sysinfo 0.31 traits `ProcessExt` and `SystemExt`

- **Severity:** HIGH
- **Where:** `performancebench-injector/pcprobe/src/collector.rs:30` (pre-fix)
...
### B-135 — `hostname` crate missing from pcprobe dependencies

- **Severity:** HIGH
- **Where:** `performancebench-injector/pcprobe/Cargo.toml` (missing)
...
### B-136 — `hostname::get().and_then(|h| h.into_string().ok())` type mismatch

- **Severity:** HIGH
- **Where:** `performancebench-injector/pcprobe/src/discovery.rs:32-34` (pre-fix)
...
### B-137 — `discovery.rs` uses `std::sync::mpsc::RecvTimeoutError` but mdns-sd 0.10 uses flume

- **Severity:** HIGH
- **Where:** `performancebench-injector/pcprobe/src/discovery.rs:115-116` (pre-fix)
...
### B-138 — `discovery.rs` uses removed `get_property()` API on ServiceInfo

- **Severity:** HIGH
- **Where:** `performancebench-injector/pcprobe/src/discovery.rs:90-96` (pre-fix)
...
### B-139 — tokio missing `rt-multi-thread` and `time` features

- **Severity:** HIGH
- **Where:** `performancebench-injector/pcprobe/Cargo.toml:20` (pre-fix)
...
### B-140 — `is_elevated()` has unused imports and uses `libc::geteuid()` without libc dependency

- **Severity:** MED
- **Where:** `performancebench-injector/pcprobe/src/collector.rs:230-240` (pre-fix)
...
### B-141 — `hostname` moved into properties vec then borrowed for ServiceInfo::new

- **Severity:** MED
- **Where:** `performancebench-injector/pcprobe/src/discovery.rs:42,48` (pre-fix)
