...
- [x] 02-PLAN.md — Auto-detected issues engine (12 rules per §6.9), session collections with flat tags + project_id, session search + multi-filter (V15-03, V15-04, V15-05)
- [x] 03-PLAN.md — Metric threshold alerts with status bar badge + auto-markers, auto session start via ADB logcat polling (V15-06, V15-07)
- [x] 04-PLAN.md — tidevice on Windows for iOS (~8 metrics), Mac proxy daemon (HTTP REST + WebSocket), Linux first-class smoke test (V15-08, V15-09, V15-10)
- [x] 05-PLAN.md — Android video recording via screenrecord (5-min chunks), video player UI with bidirectional chart scrub sync (V15-11, V15-12)
...
- [x] 04-03-PLAN.md — Frida gadget injection alternative + WebView JS memory collection + per-process network stats (V25-07, V25-08, V25-09)
- [x] 04-04-PLAN.md — ADB broadcast automation (7 commands) + iOS video recording via pymobiledevice3 DVT (V25-10, V25-11)
...
- [x] 06-02-PLAN.md — Audit logging (audit_events table + API + CSV/JSON export), team orgs/projects/membership, on-prem bare metal install script, production Docker Compose with nginx, air-gapped deployment checklist (V35-06, V35-07, V35-09) → SUMMARY: 06-02-SUMMARY.md
- [x] 06-03-PLAN.md — Enterprise dashboard UI (SSO config, user management, audit log viewer), Jira integration, thread-level CPU breakdown, Helm chart, enterprise deployment README (V35-04, V35-08) → SUMMARY: 06-03-SUMMARY.md
